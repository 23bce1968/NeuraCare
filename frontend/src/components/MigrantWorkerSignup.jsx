...
export default function MigrantWorkerSignup({ onClose, onSignin, onSuccess }) {
  ...
  const [formData, setFormData] = useState({
    name: "", email: "", phone: "", password: "", otp: "", role: "worker",
  });
  const [errors, setErrors] = useState({ name: "", email: "", phone: "", password: "", otp: "" });
  ...
  const validateDetails = () => {
    ...
    let ok = true;
    if (formData.name.trim().length < 2) { newErrors.name = "Enter a valid name"; ok = false; }
    if (!/\S+@\S+\.\S+/.test(formData.email)) { newErrors.email = "Enter a valid email"; ok = false; }
    if (!/^[0-9]{10}$/.test(formData.phone)) { newErrors.phone = "10-digit number required"; ok = false; }
    if (formData.password.length < 6) { newErrors.password = "Password must be at least 6 characters"; ok = false; }
    setErrors(newErrors);
    return ok;
  };
  ...
  const handleVerifyAndSignup = async (e) => {
    e.preventDefault();
    if (formData.otp.length !== 6) { setErrors((prev) => ({ ...prev, otp: "OTP must be 6 digits" })); return; }
    setIsSubmitting(true);
    try {
      await axios.post("http://localhost:8000/auth/signup/verify-email", {
        email: formData.email, verificationCode: formData.otp,
      });
      handleClose();
      onSignin?.();
    } catch (err) {
  ...
  return (
    <div className={`fixed inset-0 z-[9999] bg-black/40 backdrop-blur-sm flex items-center justify-center px-3 transition-opacity duration-300 ${isVisible ? "opacity-100" : "opacity-0"}`}>
      <div className={`relative w-full max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 ease-out ${isAnimating ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>

        <button onClick={handleClose} className="absolute top-3 right-3 p-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 z-10">✕</button>
        ...
        <div className="p-4 sm:p-6">
          ...
          <form onSubmit={handleVerifyAndSignup} className="space-y-3">
            ...
            {!otpSent && (
              <>
                <Input label="Full Name" placeholder="Enter your full name" icon={<User className="w-4 h-4" />}
                  value={formData.name} error={errors.name} onChange={(v) => handleChange("name", v)} />

                <Input label="Email Address" placeholder="your.email@example.com" icon={<Mail className="w-4 h-4" />}
                  value={formData.email} error={errors.email} onChange={(v) => handleChange("email", v)} />

                <Input label="Phone Number" placeholder="10-digit phone number" icon={<Phone className="w-4 h-4" />}
                  value={formData.phone} error={errors.phone} onChange={(v) => handleChange("phone", v.replace(/\D/g, ""))} />

                <div>
                  <label className="text-xs font-semibold text-gray-700 mb-1 block">Password</label>
                  <div className="relative w-full">
                    <div className="absolute inset-y-0 left-3 flex items-center text-gray-400"><Lock className="w-4 h-4" /></div>
                    <input type={showPassword ? "text" : "password"} placeholder="Min. 6 characters"
                      ...
                      className={`w-full pl-10 pr-10 py-2 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 ${errors.password ? "border-red-400" : "border-gray-300"}`} />
            ...
            {otpSent && (
              <>
                ...
                <Input label="Verification OTP" placeholder="Enter 6-digit OTP" icon={<Shield className="w-4 h-4" />}
                  value={formData.otp} error={errors.otp} onChange={(v) => handleChange("otp", v.replace(/\D/g, "").slice(0, 6))} />
                ...
                <button type="button"
                  onClick={() => { setOtpSent(false); setFormData((p) => ({ ...p, otp: "" })); setErrors((p) => ({ ...p, otp: "" })); }}
          ...
          <div className="text-center mt-4 text-xs text-gray-600">
            ...
            <button onClick={() => { onClose(); setTimeout(onSignin, 250); }} className="text-emerald-600 font-semibold hover:underline">Sign in</button>
          </div>
        </div>
      </div>
    </div>
...
function StepDot({ active, done, label }) {
  return (
    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all
      ${done ? "bg-emerald-500 border-emerald-500 text-white" : active ? "border-emerald-500 text-emerald-600 bg-white" : "border-gray-300 text-gray-400 bg-white"}`}>
    ...
    <div>
      ...
      <div className="relative w-full">
        ...
        <input disabled={disabled} placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)}
          className={`w-full pl-10 pr-3 py-2 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 ${error ? "border-red-400" : "border-gray-300"} ${disabled ? "bg-gray-100" : ""}`} />
      </div>
